<x-form-gym>
    ...
    <div class="p-8 sm:ml-64">
        <p class="text-2xl font-bold">Masukkan foto gym terbaik Anda</p>
        <p>Foto yang menarik adalah hal pertama yang diperhatikan oleh calon pelanggan.</p>

        <div class="mt-5">
            <form action="{{ route('gym.store.foto') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="flex flex-col items-center justify-center w-1/2 mt-2">
                    <div class="relative w-full h-48">
                        @if(session('gym_foto.foto_gym1'))
                        <img src="{{ asset('storage/' . session('gym_foto.foto_gym1')) }}" id="preview_foto1" class="absolute inset-0 object-cover w-full h-full" alt="Pratinjau Foto Pertama" />
                        @else
                ...
                <div class="flex flex-col items-center justify-center w-1/2 mt-2">
                    <div class="relative w-full h-48">
                        @if(session('gym_foto.foto_gym2'))
                        <img src="{{ asset('storage/' . session('gym_foto.foto_gym2')) }}" id="preview_foto2" class="absolute inset-0 object-cover w-full h-full" alt="Pratinjau Foto Kedua" />
                        @else
                ...
                <div class="flex flex-col items-center justify-center w-1/2 mt-2">
                    <div class="relative w-full h-48">
                        @if(session('gym_foto.foto_gym3'))
                        <img src="{{ asset('storage/' . session('gym_foto.foto_gym3')) }}" id="preview_foto3" class="absolute inset-0 object-cover w-full h-full" alt="Pratinjau Foto Ketiga" />
                        @else
                ...
                <div class="mt-5 flex justify-end w-1/2">
                    
                    <button type="submit" class="text-gray-900 bg-yellow-300 border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5">Lanjutkan</button>
                </div>
            </form>
        </div>
    </div>
